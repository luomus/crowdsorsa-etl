...
zip <- utils::unzip("data2025.zip", exdir = tmp)
...
for (i in seq_len(nrow(data))) {
  ...
  document[[c("publicDocument", "gatherings")]][[1L]] <- list(
    ...
    eventDate = list(
      begin = jsonlite::unbox(as.character(date)),
      end = jsonlite::unbox(as.character(date))
    ),
  ...
  post <- httr2::req_headers(post, "API-Version" = "1")
  post <- httr2::req_headers(
    post, Authorization = paste("Bearer", Sys.getenv("FINBIF_ACCESS_TOKEN"))
  )
  post <- httr2::req_body_json(post, document)
  post <- httr2::req_error(post, is_error = function(resp) FALSE)
  ...
  response_i[["body"]] <- rawToChar(response_i[["body"]])
  response_i[["request"]][["state"]] <- NULL
  response_i[["headers"]] <- unclass(response_i[["headers"]])
  response_i[["request"]] <- unclass(response_i[["request"]])
  response_i[["request"]][["headers"]] <- unclass(
    response_i[["request"]][["headers"]]
  )
  response_i[["request"]][["headers"]][["Authorization"]] <- NULL
...
cat(response, file = "logs2025.json")
